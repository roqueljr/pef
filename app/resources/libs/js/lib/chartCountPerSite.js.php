...
async function perSite() {
    const obj = {
        ctxt: 'get'
    }

    try {
        const data = await fetch('/0/app/api/v1/tableCountPerSite.php', {
            method: 'POST',
...
async function viewPieChart() {
    ...
    var colorIndex = 0;
    var pieData = [];
    const trees = await perSite();
    const labels = trees.map(tree => tree.sitio);
    ...
    const canvass = document.getElementById('pieChart');
    if (canvass) {
