...
@section('content')
    ...
    <div class="container websites">
        <h5 class="mt-5">Manage Websites</h5>
        <div class="d-flex flex-wrap justify-content-center align-items-center justify-content-md-start">
            @foreach($websites as $web)
                <div class="card m-3 position-relative z-0" style="width: 18rem">
                    <img src="<?= $web['logo'] ?>" class="card-img-top img-thumbnail" alt="clogo"
                        style="height: 240px; width: 100%;">
                    <div class="card-body">
                        <h5 class="card-title"><?= $web['name'] ?></h5>
                        <p class="card-text text-truncate" style="max-width: 100%;" data-bs-toggle="tooltip"
                            title="<?= htmlspecialchars($web['description']) ?>">
                            <?= htmlspecialchars($web['description']) ?>
                        </p>
                        <a href="<?= $web['path'] ?>" target="_blank" class="btn btn-light">
                            Open
                            <i class="bi bi-arrow-right"></i>
                        </a>
                    </div>
